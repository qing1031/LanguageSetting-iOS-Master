...
- (void) viewCanBeSee{
    if ([[self.view subviews] count] == 0)
        [self drawView];
    [self.myNavigationController setTitle:@"快捷登录"];
}
...
- (void)drawView{
    ...
    authenticate = [[UITextField alloc] initWithFrame:CGRectMake(25, (height - 40) / 2, [InitData Width] - 180, 40)];
    authenticate.tag = 301;
    authenticate.delegate = self;
    authenticate.backgroundColor = [UIColor whiteColor];
    authenticate.placeholder = @"请输入验证码";
    authenticate.delegate = self;
...
- (void) timeFireMethod{
    secondsCountDown--;
    NSString *str = [NSString stringWithFormat:@"已发送(%d秒)",secondsCountDown];
