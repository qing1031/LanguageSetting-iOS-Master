...
@interface PublicResumeViewController ()<UITextFieldDelegate, EduDelegate,MutableMenuDelegate, IntruduceDelegate>{
...
@implementation PublicResumeViewController
...
- (void) viewCanBeSee{
    ...
    [self.myNavigationController setTitle:MYLocalizedString(@"发布微简历", @"Release micro resume")];
...
- (void) save:(UIButton*) but{
    ...
    if (menBut.selected){
        resume.sex = 1;
        resume.sex_cn = MYLocalizedString(@"男", @"Man");
    }else{
        resume.sex = 2;
        resume.sex_cn = MYLocalizedString(@"女", @"Woman");
    }
    ...
    resume.tpl = [((UILabel*)[array objectAtIndex:6]).text intValue];
    resume.telephone =((UILabel*)[array objectAtIndex:8]).text;
    resume.email =((UILabel*)[array objectAtIndex:9]).text;
    
    [InitData isLoading:self.view];
    but.userInteractionEnabled = NO;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        //后台代码
        T_Resume *tresume = [[[ITF_Other alloc] init] simpleResumeOperationByID:0 andResume:resume];
