...
//        UrlImageButton *imageView = [[UrlImageButton alloc]initWithFrame:CGRectMake(0, 0, 320, imageH)];
  ...
  //   [imageView setImageFromUrl:YES withUrl:imageName];
        if ([[imageName componentsSeparatedByString:@":"] count] > 1){
            NSData *data = [[[DealCacheController alloc] init] getImageData:imageName];
...
//
    for (int i = 0; i <= imageArr.count +1; i++) {
        ...
        //创建内容对象
        CGRect titleRect = CGRectMake(0, 150, 320, 30);
        BMImageView *imageView =  [[BMImageView alloc]initWithImageName:imageURL title:title x:WIDTH*i tFrame:titleRect iHeight:imageSV.frame.size.height titleHidden:ISTITLEHIDDEN];
